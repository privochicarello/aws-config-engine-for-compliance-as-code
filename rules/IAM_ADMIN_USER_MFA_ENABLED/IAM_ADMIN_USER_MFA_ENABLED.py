...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for policy_name in inline_policy_names:
        ...
        if is_statements_include_full_star_allow(policy_document['Statement']) and not mfa_enabled(iam_client, user_name):
            return build_evaluation_from_config_item(configuration_item, "NON_COMPLIANT", annotation='The user "' + user_name + '" has full star allow permissions and no MFA devices.')
    ...
    for policy_arn, policy_name in managed_policy_arn_and_name.items():
        ...
        if is_statements_include_full_star_allow(get_policy_version['PolicyVersion']['Document']['Statement']) and not mfa_enabled(iam_client, user_name):
            return build_evaluation_from_config_item(configuration_item, "NON_COMPLIANT", annotation='The user "' + user_name + '" has full star allow permissions and no MFA devices.')
...
def mfa_enabled(iam_client, user_name):
    list_user_mfa  = iam_client.list_mfa_devices(UserName=user_name)
    if len(list_user_mfa['MFADevices']) == 0:
        return False
    else:
        return True
