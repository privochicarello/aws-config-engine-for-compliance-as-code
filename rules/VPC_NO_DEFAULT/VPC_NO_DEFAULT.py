...
Description:
    Check whether Default VPCs exist
...
+------------------+----------------+--------------------------------------------------------------------------------------------+
| WhiteListedVPC   | Optional       | This parameter can be used to white-list Default VPCs, the white-listed VPCs will not      |
|                  |                | be evaluated and be returned as COMPLIANT. Separate multiple VPCs by a comma.              |
+------------------+----------------+--------------------------------------------------------------------------------------------+                                                                                          |

Feature:
  In order to: Check if Default VPCs exist
           As: a Security Officer
       I want: to ensure that all Default VPCs are removed

Scenarios:

  Scenario 1:
    Given: The parameter WhiteListedVPC is not valid
...
ALLOWED_PARAMETER_NAMES = ['WhiteListedVPC']
...
def evaluate_compliance(event, configuration_item, rule_parameters):
    ...
    default_id_list = get_all_default_vpc_id(ec2_client)
    vpc_list = get_all_vpc(ec2_client)
    print(vpc_list)
    ...
    for vpc_id in vpc_list:
        ...
        if vpc_id in default_id_list:
            evaluations.append(build_evaluation(vpc_id, 'NON_COMPLIANT', event, annotation='This a default VPC'))
            continue    
        
        evaluations.append(build_evaluation(vpc_id, 'COMPLIANT', event, annotation='This is not a default VPC.'))

    return evaluations

def get_all_vpc(ec2_client):
...
def get_all_default_vpc_id(ec2_client):
...
def lambda_handler(event, context):
    ...
    while(evaluation_copy):
        AWS_CONFIG_CLIENT.put_evaluations(
            Evaluations=evaluation_copy[:100], ResultToken=resultToken, TestMode=testMode)
