...
aws cloudformation deploy --stack-name complianceascode-engine-datalake --template-file ./rulesets-build/compliance-account-analytics-setup.yaml --no-fail-on-empty-changeset --parameter-overrides CentralizedS3BucketConfig="${centralizedbucketconfig[@]}" CentralizedS3BucketComplianceEventName="${centralizedcomplianceevent[@]}" KeyListGeneratedByFirehose="${keylistfirehose[@]}" ColumnKeyList="${columnkeylist[@]}" AccountList="${accountlist[@]}" LocationAccountListCSV="${locationaccountlist[@]}"

response=$(aws cloudformation list-change-sets --stack-name complianceascode-engine-datalake --query "Summaries[*].ChangeSetName" --output text)
declare -a changesets=($response)
for changeset in "${changesets[@]}"; do
  aws cloudformation delete-change-set --change-set-name $changeset --stack-name complianceascode-engine-datalake
done
